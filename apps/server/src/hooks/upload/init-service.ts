...
import type { UploadPolicyKey, UploadSessionInfo } from '@repo/types';
...
export async function initUpload(
  ...
  if (!Number.isSafeInteger(input.size) || input.size <= 0) {
    throw createUploadError('UPLOAD_OBJECT_MISMATCH', '文件大小必须为正整数');
  }
  if (input.size > policy.maxFileSizeBytes) {
    throw createUploadError('UPLOAD_FILE_TOO_LARGE', '文件超过策略大小上限');
  }
  if (
