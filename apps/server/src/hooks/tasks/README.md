...
不得直接访问任务表、attempt、lease、存储函数、Dispatcher 或 Worker 进程监督。
...
`script` 是服务端业务代码构造的内部模块 URL，不接受 HTTP 请求传入。任务模块直接动态导入该
URL；模块默认导出执行函数，函数只接收一个对象：
...
- `task.start()`：恢复异常中断任务并启动 Dispatcher，只由服务启动代码调用。
...
## Dispatcher、Worker 与恢复

Dispatcher 在全局并发和同名任务并发都有空位时原子领取任务。主进程把已持久化的 `script` 和
`data` 交给独立 Node.js 子进程；子进程直接动态导入脚本，不依赖注册表或创建任务进程的内存。
父进程负责 PID、heartbeat、超时、stdout/stderr 日志及 `SIGTERM` 到 `SIGKILL` 的终止升级。

Dispatcher 启动时和运行期间都会恢复 lease 过期的任务。任务语义是 at-least-once，业务脚本必须
保证重复执行安全。

## 内部文件

- `task.ts`：唯一公共入口。
- `database.ts`：`TaskDatabase`，集中管理任务查询、attempt、lease、日志和状态迁移。
- `dispatcher.ts`：主进程中的 `TaskDispatcher` 管理实例调度，`TaskExecution` 管理单任务 Worker 生命周期。
- `worker-entry.ts`：Worker 子进程入口和脚本执行包装。
