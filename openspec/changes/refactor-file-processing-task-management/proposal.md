...
当前管理端把通用文件、上传会话、文档处理和知识库关联分别暴露给用户，服务端也由多个 hook 和多套任务状态承载同一条文件处理流程，导致用户操作割裂、任务记录难以统一查看，代码入口和职责不够清晰。需要把业务收敛为“上传一个文件，可选择是否进入 RAG；一次预处理与 RAG 执行就是一个可追溯任务”，并复用现有任务中心统一管理等待、执行中和历史任务。

## What Changes

- 管理端以单一“文件管理”页面作为文件业务入口，移除“通用文件”“上传会话”和独立 RAG 文档操作页签；上传会话仅作为上传组件内部的恢复状态。
- 上传文件时允许选择是否进入 RAG，并允许配置默认选择；选择进入 RAG 时指定目标知识库，文件验证成功后自动创建一个完整文件处理任务。
- 未选择进入 RAG 的文件只保存到文件列表，用户可在文件操作中手动创建处理任务。
- 将一次文件预处理、内容切分和 RAG 接入定义为一个任务，内部阶段只用于进度、错误定位和重试，不在任务中心平铺成多个独立任务。
...
- `unified-file-processing-task`: 定义一个文件一次完整预处理与 RAG 接入对应一个任务、阶段进度、执行次数、并行处理、幂等、取消和重试行为。
