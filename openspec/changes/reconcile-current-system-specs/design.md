...
- `add-schema-form-component` 与 `harden-admin-role-permissions` 没有后继 change 推翻其 capability，实施前完成代码对照后正常归档，由各自 delta 生成主 specs。
...
- [独立 change 与基线重复 capability] → 权限与 SchemaForm 不进入本 change，由原 change 正常归档；文档链全部由本基线接管。
...
2. 对照当前代码复核 `add-schema-form-component` 与 `harden-admin-role-permissions`，分别正常归档。
