...
本 change 已被后续文档中心迭代及 `reconcile-current-system-specs` 基线取代。其通用上传传输能力由新 `generic-file-upload-management` 承接；“上传只返回 fileId”、File 中心、旧多模式预览、`file_variants`/`file_references` 等要求不再代表当前系统事实。本 change 仅作为演进历史使用 `--skip-specs` 归档。
