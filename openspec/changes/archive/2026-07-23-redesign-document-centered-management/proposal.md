...
- 文档解析、标准化和 Segment 生成直接属于 DocumentVersion，同一版本与配置只处理一次；多个知识库复用该内容结果，RAG 目录只维护关系、版本发布和未来索引边界。
...
- 任务运行时：复用现有任务与 worker，预览和版本内容任务都绑定 `documentVersionId`，通过任务类型区分并独立重试。
- API 与权限：现有 `/documents/file-*`、`dataset-document-*` 及文件处理接口需要迁移或下线，属于对管理端和潜在外部调用方的兼容性变更。

## Superseded

本 change 是文档中心最终基线的主要来源，其有效要求已完整吸收到 `reconcile-current-system-specs` 的文档生命周期、版本、图片预览和 RAG 版本 capability。为避免相同 ADDED capability 重复进入主 specs，本 change 仅作为演进历史使用 `--skip-specs` 归档。
