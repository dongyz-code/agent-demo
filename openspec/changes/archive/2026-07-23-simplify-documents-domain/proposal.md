...
本 change 的精确导入、真实功能边界、worker lease 和从 reading 重试规则已由 `reconcile-current-system-specs` 的 `documents-domain-boundary` 与 `document-processing-task` 承接；其中声称需要补建的服务端 tests 已被后续删除测试决策取代。本 change 仅作为演进历史使用 `--skip-specs` 归档。
