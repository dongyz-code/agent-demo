> 2026-07-23 最终决策：权限以当前单 route 单权限 key 实现为准，不新增动态或多权限规则；重新打开的任务用于把 spec、design 与集中式 `preHandler` 实态对齐后归档。
...
- [x] 2.2 权限声明保持单 key，不增加当前无实现和无明确消费者的多 key、动态函数或策略规则。
- [x] 2.3 新增统一 admin 权限上下文 helper，支持系统管理员、启用角色权限聚合、未知权限 key 过滤和权限集合去重。
- [x] 2.4 明确身份认证 `onRequest` 与唯一集中权限 `preHandler` 的执行顺序，确保权限规则在业务 handler 前执行，且 spec、design 与实际 hook 生命周期一致。
- [x] 2.5 调整登录和 token 校验权限聚合逻辑，只返回启用角色的有效权限，并保持系统管理员 `sys_admin` 行为不变。
- [x] 2.6 新增角色权限 payload 校验工具，角色创建和更新提交未知权限 key 时返回非法参数错误。
- [x] 2.7 确认 `/sys/**` routeHandler 均已声明单个页面或操作权限，角色、用户和应用复合更新接口与当前 route 级权限实现一致。
