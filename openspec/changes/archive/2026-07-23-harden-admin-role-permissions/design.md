...
- 在服务端保持身份认证与业务授权职责分离：认证在 `onRequest` 完成，唯一的集中式权限 `preHandler` 随后执行；前端隐藏按钮只是体验层优化，不是安全边界。
...
### 4. 后端权限注册在 routeHandler，由认证后的集中 hook 统一执行

扩展 `apps/server/src/router/utils.ts` 的 `routerHandler` 入参，增加 `permission` 字段。该字段支持：

- 单个权限 key。

`routeHandler` 只把权限 key 写入 Fastify route config，不在业务 handler 中执行鉴权。现有 `initRoutes` 已经把 `authentication.authentication(req)` 注册为 Fastify `onRequest`，继续由 `router/index.ts` 安装唯一的权限 `preHandler`：它从 route config 读取 key，使用已建立的 `request.auth` 权限上下文执行断言。这样权限声明跟接口定义在一起，身份识别和业务授权各自保持单一职责，业务 handler 仍然干净。

现阶段不实现多 key、动态函数或通用策略 DSL。角色、用户和应用的现有复合更新接口以 route 的单个 update 权限覆盖整个请求；如果以后确实需要服务端字段级授权，应先拆分明确用例再提出 change。权限不足必须抛出 `ROOT_ERROR('认证: 权限不足')` 并映射为 403，不能被身份认证失败逻辑统一压成 401。
...
读取类接口通常声明页面权限，写入类接口声明该 route 当前实际执行的单个动作权限。角色管理服务端使用 `actions.role.create`、`actions.role.update` 和 `actions.role.delete`；`actions.role.toggle` 与 `actions.role.assign-permission` 保留给当前前端交互控制，不声明为服务端字段级守卫。
...
4. 扩展 server `routeHandler` 的单权限 key 字段，由认证后的集中式 `preHandler` 校验。
5. 修改权限聚合，过滤禁用角色和未知 key，并通过受影响包 lint/build 验证类型契约。
6. 在系统管理接口定义处声明当前 route 的单个页面或操作权限。
...
- 后续如果出现必须由服务端区分的复合写字段，再评估拆路由或动态规则；当前以单 route 单权限为准。
