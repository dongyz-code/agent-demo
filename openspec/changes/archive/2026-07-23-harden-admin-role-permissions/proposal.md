...
- server 接口权限注册到 `routeHandler` 中，由唯一的集中式 `preHandler` 在身份认证完成后读取 route config，并在业务 handler 前拦截。
...
- `apps/server`: 扩展 `routeHandler` 的单权限 key 注册能力，由集中式权限 hook 在认证后执行校验，调整登录权限聚合和角色权限 payload 校验。
