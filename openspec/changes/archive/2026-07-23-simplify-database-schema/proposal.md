...
2026-07-23 规范治理确认本 change 的 26 表基线、五表退役清单和 `file_variants` 保留假设已经过期，不能继续执行剩余任务。工作已按当前实态拆分：

- 已完成的数据库出口收敛由 `simplify-database-access` 承接；
- 当前唯一仍存在的重复投影 `file_upload_parts` 由 `remove-upload-part-projection` 承接；
- 外键 descriptor、DDL、catalog diff 与受控 apply 由 `add-drizzle-foreign-key-lifecycle` 承接。

本 change 仅作为历史使用 `--skip-specs` 归档，未完成任务不得解释为已交付。
