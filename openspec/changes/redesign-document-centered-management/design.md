...
- 处理任务扩展只保留一套。复用现有 `file_processing_tasks`，只保存 File、Document、DocumentVersion、task type、执行序号和配置版本；内容任务不保存单个 datasetId。删除未实际使用的 `document_processing_jobs`，不新增第三套任务表。

不采用预览批次表：每个版本首期只需要一套页面，生成器升级时重新生成并替换即可。

不采用 RAG result 表：首期只需要知道知识库当前使用哪个版本、正在处理哪个版本。任务历史已经能回答某次处理是否成功。

文档解析、标准化和 Segment 是 DocumentVersion 的内容产物，不按知识库重复生成。内容任务以 `documentVersionId + processingConfigVersion` 为幂等边界；同一版本加入多个知识库时只执行一次，成功后批量发布所有仍以该版本为 pending 的关系。
...
解析、标准化和 Segment 生成不属于某个 Dataset，统一由 `document/content` 处理一次。RAG 只消费版本内容结果并维护各知识库关系的生效状态；当前尚未实现 Embedding 和向量索引，因此不得把内容处理成功描述为已经完成向量检索建设。
...
检索必须用关系的 activeVersionId 过滤 Segment/向量结果。Segment 绑定 documentVersionId；未来索引候选再携带 datasetId、documentId 和 documentVersionId，未成为 active 的版本不可见。

历史回切时先检查该版本是否已有成功内容任务：已有结果则把所有对应关系直接切换 active；没有则分别设置 pending，但只创建一个版本内容任务，旧 active 继续服务。
...
│   ├── remove.ts           逻辑删除和异步清理
│   └── content/            版本级解析、标准化、Segment 与唯一内容任务
...
│   └── pages.ts            页面窗口和签名
├── rag/
│   ├── assignment.ts       关系集合变更与内容任务触发
│   └── relations.ts        多知识库关系和 active/pending 批量切换
...
- [同一版本加入多个知识库导致重复解析] → 内容任务不包含 datasetId，以版本和处理配置幂等；知识库关系只复用结果并独立发布。
