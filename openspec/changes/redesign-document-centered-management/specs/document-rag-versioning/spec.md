...
### Requirement: 迟到任务不得覆盖最新目标
RAG 任务发布时 MUST 使用 `pendingVersionId` 条件更新。用户连续上传或切换版本后，旧任务即使成功也不得覆盖更新后的 pendingVersion。
...
用户把历史版本设为文档当前版本时，系统 MUST 为每个已关联知识库检查已有成功任务；已有结果可直接切换，否则把该版本设为 pendingVersion 并继续使用旧 activeVersion。

#### Scenario: 历史版本需要重新处理
- **WHEN** 当前展示版本切换到尚无成功 RAG 结果的历史版本
- **THEN** 各知识库创建或复用处理任务，旧 activeVersion 保持服务直到成功

### Requirement: RAG 任务按知识库和版本幂等
系统 MUST 复用现有任务机制，以 Dataset、DocumentVersion 和处理配置作为活动任务幂等边界。

#### Scenario: 重复触发同一 RAG 任务
- **WHEN** 多个入口同时触发相同知识库和版本处理
- **THEN** 系统返回同一个活动任务且不重复写入结果
