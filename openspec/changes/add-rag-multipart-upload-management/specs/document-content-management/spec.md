...
系统 MUST 在单一 `hooks/documents` 域内按功能管理文档、版本、解析、标准化、Segment 和处理任务。文档聚合读写以及版本内容处理归 `document`，知识库关系与版本发布归 `rag`，上传编排归 `upload`；普通单表查询和简单更新直接由 route 使用 ORM，不得恢复独立顶层 `hooks/document` 或根 barrel。
...
### Requirement: 文档内容处理通过内部源文件边界生成版本化 Segment
`document/content` MUST 通过 `storage/source` 的内部源文件能力读取已验证版本，并生成版本化 `DocumentSegment`，包含确定性标识、内容、Embedding 输入、标题路径、页码、位置和内容 Hash。该内部能力不得通过根 barrel 或 route 暴露。

#### Scenario: RAG 消费文档
- **WHEN** 文档版本需要生成可供知识库消费的内容
- **THEN** 内容 runner 精确导入源文件和解析能力，不经过根公共入口，也不让 route 读取源文件或 parser 内部实现
